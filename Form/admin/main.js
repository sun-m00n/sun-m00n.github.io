...
function del_RECORD(id) {
    alert(`Delete Form ${id[0]}`)

    fetch(api_url, {
        method: 'POST',
        body: JSON.stringify({ type: "del", ids: id }),
    })
        .then(response => response.json())
        .then(function (data) {
...
async function render(data) {
    ...
    <td><svg data-formno="${data['Form No.']}" onclick="get_RECORDS([this.getAttribute('data-formno')])" xmlns="http://www.w3.org/2000/svg" shape-rendering="geometricPrecision" text-rendering="geometricPrecision"
